...
def dbscan_cluster(points, min_points=5, epsilon=0.1):
    ...
    corePts = np.sum(np.sum((points[None, :, :] - points[:, None, :]) ** 2.0, axis=2) < epsilon ** 2.0,
                     axis=1) >= min_points
    plt.scatter(points[corePts, 0], points[corePts, 1], c='green', s=5)
    plt.scatter(points[~corePts, 0], points[~corePts, 1], c='red', s=5)
    ...
    plt.scatter(points[~reachable, 0], points[~reachable, 1], c='red', s=5)
